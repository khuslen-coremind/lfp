...
function PostForm(props) {
	...
	const handleReject = () => {
		showNotification({
			...
			disallowClose: true,
			autoClose: 5000,
			title: "Sorry, only images (.png, .jpg, .jpeg, .gif) and videos (.mp4) accepted.",
			color: "red",
			loading: false,
		});
	};
			...
			<Group direction="column" style={{ width: 600 }} position="apart" mr="xl">
				...
				<Group sx={{ justifySelf: "end", alignSelf: "end" }}>
					...
					<Button
						variant="filled"
						type="submit"
						onClick={isDraft ? handleSubmit(false, false, draftId) : handleSubmit(true)}
						loading={loading}>
						POST
					</Button>
				</Group>
...
const rules = [
	...
	{ title: "Self promo is accaptable, within reason", content: " " },
];
