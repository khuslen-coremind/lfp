...
function Room(props) {
	...
	useEffect(() => {
		const client = io(`http://${API_URL}`);
		client.on("connect", () => {
			console.log("connected");
		});
		userId &&
			client.emit("join", { userId, roomId }, () => {
	...
	useEffect(() => {
		getMessageHistory()
			.then((res) => {
				...
				setRoomMessages(...roomMessages, reversed);
			})
			.catch((err) => {
				console.log(err);
			});
	...
	useEffect(() => {
		...
		socketClientRef.current.on("receive_room_messages", (data) => {
	...
	return (
		<div style={{ position: "relative" }}>
			<ActionIcon onClick={handlePrevious} style={{ position: "absolute", left: -65 }}>
				<IoReturnUpBack size={55} />
			</ActionIcon>
			<div style={{ maxWidth: 960, width: 960 }}>
				<Group direction="column" spacing="xs" style={{ width: "100%" }}>
					<Paper withBorder shadow="lg" p="xs" pl="md" style={{ width: "inherit" }}>
	...
	return (
		<Group direction="column" position="center">
			...
			<Stack style={{ width: "100%" }} spacing="sm">
				<User
					user={{
						imgSrc: "https://practicaltyping.com/wp-content/uploads/2020/08/gon.png",
						name: "hihe",
					}}
				/>
				<User
...
const RoomMessage = ({ authorId, message, userId }) => {
	return parseInt(authorId) === userId ? (
		<Group noWrap mt="xs" style={{ width: "100%" }} position="right" spacing="xs">
			<Text
				sx={(theme) => ({
