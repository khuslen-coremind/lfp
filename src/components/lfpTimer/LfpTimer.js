...
function LfpTimer({ roomDetail }) {
	...
	useEffect(() => {
		const dateNow = moment(moment.now()).format("YYYY-MM-D hh:mm:ss");
		const targetTime = moment(roomDetail.targetTime).format("YYYY-MM-D hh:mm:ss");
		if (moment(dateNow).add(5, "m").diff(targetTime, "minute") > 5) {
			setNow(false);
		} else {
			setNow(true);
			setInterval(() => {
